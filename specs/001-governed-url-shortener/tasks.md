...
- **T011** [P] — `domain_short_link` table + model
  ...
  **Risk**: schema drift from data-model.md — mitigated by T124 (schema-vs-doc check)
...
- **T020** [P] — Bounded-retry wrapper for `SQLITE_BUSY`/`SQLITE_LOCKED`
  **Req**: FR-401–FR-406 **Scn**: — **ADR**: ADR-0007 **Path**: `src/persistence/retry.py`
...
- **T023** — Implement `POST /short-links` (happy path)
  **Req**: FR-101, FR-102 **Scn**: US1 AS1 **ADR**: ADR-0004 **Path**: `src/api/routers/short_links.py`, `src/domain/short_link.py`
...
- **T029** — Implement `GET /{shortCode}` resolution
  **Req**: FR-103 **Scn**: US1 AS2 **ADR**: — **Path**: `src/api/routers/redirect.py`
...
- **T040** — MTTR instrumentation: timestamps, recovered-event population, exclusions
  **Req**: — **Scn**: — **ADR**: ADR-0008 **Path**: `src/observability/metrics.py`
  **Prereq**: T106 (audit_events table, Group 26) **Dep**: T106 **P**: no
...
**Checkpoint**: analytics durability, uncertainty-handling, and MTTR definition all real and tested — this closes out the domain/analytics work; Group 10 may proceed.
...
- **T048** — Real `claude` CLI invocation wrapper (corrected mechanism)
  ...
  **Risk**: reintroducing the wrong `--cwd`-flag syntax would be a regression of an already-caught, already-fixed defect
...
- **T051** — Policy-evaluation stage (produces `PASS`/`FAIL`/`EXCEPTION-REQUESTED`/`NOT-APPLICABLE`)
  **Req**: FR-303, FR-304 **Scn**: — **ADR**: — **Path**: `src/policy/evaluator.py`
...
- **T052** — `FAIL` mechanically blocks downstream progression
  ...
  **Prereq**: T051, T054 (scheduler) **Dep**: T051 **P**: no
...
**Checkpoint**: policy enforcement real — Group 14 may proceed.
...
- **T053** — `policy_exception` schema + approval-gated creation
  ...
  **Prereq**: T050, T086 (approval mechanism, Group 19) **Dep**: T050 **P**: no
  ...
  **Docs**: data-model.md **Trace**: FR-305 **Risk**: depends on Group 19's approval mechanism, which carries ADR-0006's unresolved isolation — an accepted, disclosed limitation, not silently ignored **Done**: all fields required, expiry enforced **Human**: yes — exception creation requires the approval mechanism from Group 19
...
- **T060** — Effect reconciliation (`check_effect()` on exception/timeout/stale-lease)
  **Req**: — **Scn**: — **ADR**: ADR-0005, ADR-0007 **Path**: `src/orchestration/reaper.py`
  ...
  **Docs**: Human Gate 4 spike results (already retains the real command/exit-code evidence for the isolated logic) **Trace**: ADR-0005 Rev. 3, Human Gate 4 spike (Scenarios A/B/C, all real PASS)
...
- **T063** — Append-only `decision_lineage` writer
  ...
  **Docs**: none **Trace**: FR-503 **Risk**: enforcement is currently code-review convention, not a DB constraint — disclosed, not solved here **Done**: append works, no update/delete function exists **Human**: no
...
- **T065** — Role-checked approval endpoints (`reviewer_approver`/`release_owner`)
  **Req**: FR-307, FR-312 **Scn**: US3, US4 **ADR**: ADR-0006 **Path**: `src/api/routers/approvals.py`
  **Prereq**: T064 **Dep**: T064 **P**: no
  **Artifact**: approval endpoints **TDD**: test-first, both roles, both gate types **Validation**: `uv run pytest tests/contract/test_approvals.py`
  **Docs**: contracts/openapi.yaml (already specifies these endpoints) **Trace**: FR-307, FR-312
  ...
  **Done**: role-mismatch, revision-mismatch, unauthenticated cases all correctly rejected **Human**: no (the endpoint itself doesn't need a human gate to *build*)
...
**Checkpoint — carries an explicit disclosed limitation forward, not a silent gap**: approval-gate *logic* (roles, revision-binding, timeout) is real and tested. Approval-gate *credential isolation* (ADR-0006) remains an accepted, documented risk. Group 20 may proceed.
...
- **T070** — Retry-safety gate (idempotent-vs-uncertain, reconciliation-required)
  **Req**: — **Scn**: — **ADR**: ADR-0007 **Path**: `src/orchestration/scheduler.py`
  ...
  **Docs**: ADR-0007 (already corrected from the withdrawn "in-band exception is automatically safe" claim) **Trace**: ADR-0007 Rev. 3 finding #11 **Risk**: this is the exact defect Round 3 fixed — a regression here reopens it **Done**: gate enforced for every retry path **Human**: no

- **T071** [P] — Corrected backoff (3 attempts, 1s+2s waits)
  **Req**: — **Scn**: — **ADR**: ADR-0007 **Path**: `src/orchestration/retry_policy.py`
  ...
  **Docs**: ADR-0007 (already corrected from the miscounted "1s/2s/4s") **Trace**: `PVT-002`, provisionally approved, unverified **Risk**: none beyond the miscounting Round 3 already caught **Done**: exactly 2 waits observed across 3 attempts **Human**: no
...
- **T072** [P] — Realistic per-subprocess timeouts (5s/300s/600s)
  ...
  **Docs**: ADR-0007 **Trace**: `PVT-003`, provisionally approved, unverified **Risk**: none beyond the provisional-number caveat **Done**: each type's timeout independently configurable and enforced **Human**: no
...
- **T073** — Fallback-on-`failed_permanent` (defined per stage or safe-stop)
  ...
  **Docs**: none **Trace**: ADR-0007 **Risk**: none **Done**: both branches tested **Human**: no
...
- **T074** — Per-operation-class rollback/compensation dispatch
  **Req**: — **Scn**: — **ADR**: ADR-0007 **Path**: `src/orchestration/compensation.py`
  ...
  **Docs**: ADR-0007 (already states this rule) **Trace**: ADR-0007, Constitution Principle VIII **Risk**: "invalid rollback claims" is an explicit guide failure trap — the test must specifically assert no rollback is attempted where infeasible **Done**: test passes for both classes **Human**: no
...
- **T075** — Safe-stop triggers (retry exhaustion, unresolved policy FAIL, stuck reconciliation)
  ...
  **Docs**: ADR-0007 **Trace**: ADR-0007, FR-405 **Risk**: none **Done**: all 3 triggers correctly reach `safe_stopped`, never a hang **Human**: no

- **T076** [P] — Safe-stop resumable only via explicit authorized action
  **Req**: — **Scn**: — **ADR**: — **Path**: `src/orchestration/scheduler.py`
  ...
  **Docs**: none **Trace**: Constitution Principle VIII **Risk**: none **Done**: test passes **Human**: no
...
- **T080** — `audit_events` append-only writer, all required fields
  **Req**: FR-601, FR-602 **Scn**: — **ADR**: ADR-0008 **Path**: `src/observability/audit.py`
  **Prereq**: T045 **Dep**: T045 **P**: no
...
- **T084** — `scripts/demo_greenfield.py`
  **Req**: FR-601, FR-605 **Scn**: Scenario A **ADR**: — **Path**: `scripts/demo_greenfield.py`
...
- **T085** — `scripts/demo_brownfield.py` + mandatory pre-change impact analysis
  **Req**: FR-605 **Scn**: Scenario B **ADR**: — **Path**: `scripts/demo_brownfield.py`
...
- **T086** — `scripts/demo_ambiguous.py`
  **Req**: FR-605 **Scn**: Scenario C **ADR**: — **Path**: `scripts/demo_ambiguous.py`
...
- **T090** — Full orchestration state-transition sweep (this is the Phase 0 stop-condition gate)
  ...
  **Prereq**: Groups 11–27 **Dep**: T060, T090 (self, see Risk) **P**: no
...
- **T096** — Run the corrected release-readiness rule against actual state
  **Req**: FR-301–FR-306 **Scn**: — **ADR**: — **Path**: n/a (procedure, `/speckit-analyze`/`/speckit-converge` territory)
...
- **T098** [P] — Requirement→Scenario→Design→ADR→Task→Code→Test→Evidence matrix
  **Req**: — **Scn**: — **ADR**: — **Path**: `docs/traceability-matrix.md` (not yet created)
...
- **Human approval markers** above are not exhaustive of every constitutional gate — they flag tasks with a *specific, named* open decision (scheme allow-list, scenario-input selection, release-readiness sign-off, ADR-0006's interim posture). Ordinary implementation work proceeds under the already-accepted plan/ADRs without a fresh gate per task, consistent with the constitution's task-group autonomy boundary.
- **99 tasks total**, organized into 42 groups matching the guide's required list exactly (including both items the guide numbered "11").
